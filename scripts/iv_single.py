...
parser.add_argument('--Vend',        required=False, default=-100, help='End voltage')
...
def find_inst():
    ...
    global smu_rsrc, pau_rsrc

    for l in lst:
        idn = get_idn(os.path.join(path, l)) 

        if 'KEITHLEY INSTRUMENTS INC.,MODEL 2400' in idn:
            smu_rsrc = 'ASRL'+os.path.join(path, l)
            print (smu_rsrc)
